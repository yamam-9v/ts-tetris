...
| 任意課題ロードマップ | 1. ゴーストピース | 未着手(計画書のみ追加) |
| 任意課題ロードマップ | 2. スマホ対応(ボタン) | 未着手(計画書のみ追加) |
| 任意課題ロードマップ | 3. ホールド機能 | 未着手(計画書のみ追加) |
| 任意課題ロードマップ | 4. 7-bagランダマイザ + ネクスト表示 | 未着手(計画書のみ追加) |
| 任意課題ロードマップ | 5. 効果音 | 未着手(計画書のみ追加) |
| 任意課題ロードマップ | 6. スマホ対応(スワイプ・フリック) | 未着手(計画書のみ追加) |
| 任意課題ロードマップ | 7. ウォールキック(SRS) | 未着手(計画書のみ追加) |
| 任意課題ロードマップ | 8. T-spin判定 | 未着手(計画書のみ追加) |
...
- マイルストーン / ステップ: 15. リポジトリを公開する(README整備の追加、進行中)
- やったこと:
  - ユーザーから「READMEにClaude CodeをOutput Style Learningで活用したことを書いた方がいい」との提案。単に「AIに書かせた」という誤解を避けるため、Learningスタイルの実態(型定義・ロジック本体は自分で実装し、Claudeはレビュー・解説に徹する仕組み)を正確に書くことを勧め、「## 学習プロセスについて」への追記として位置づけた。
  - ユーザーが下書きを作成。Claudeがレビューし、①参照リンクの箇条書きとClaude Code活用の説明が空行で分断され話の順序が不自然、②言い回しの硬さ(「Output Style Learningにした上で活用した」)、③「スタイル」と「モード」の表記ゆれ、④一文に4要素を詰め込みすぎ、⑤「web記事」の表記、を指摘。
  - ユーザーから「カギカッコは使いたくない」「句点を使いたくないことにより文章の区切り方が難しい」という制約が示され、既存の文体(箇条書き・行末スペースによる改行で句点を使わない)を活かしつつ、固有名詞はバッククォート(`` `Learning` ``のように、他のセクションの`strict: true`等と統一)で代替する方針を提案。ユーザーが了承し、「Claude CodeのOutput Styleを`Learning`にした上で活用した」「このスタイルでは型定義･ロジックは自分で実装しClaudeはレビュー･解説に徹する」「まずは自分で考え､分からないところは聞き､MDNやWeb記事も読みながら型や関数について学習を進めた」の3行+参照リンク2行という構成に整理。
...
- マイルストーン / ステップ: (計画書の更新のみ。第4マイルストーン完了後の任意課題ロードマップ新設)
- やったこと:
  - 全17ステップ完了後、ユーザーから追加課題の相談(T-spin判定、ホールド機能、ネクスト表示、7ピースが連続しないランダマイズ、ウォールキック、ゴーストピース、効果音、React導入、スマホ対応)を受けた。今回は相談・計画書更新のみで、実装は着手していない。
  - React導入について、Canvas 2D APIへの直接描画と`requestAnimationFrame`によるゲームループという実装の核がReact導入後もReactの管轄外にとどまるため「重すぎるとまでは言えないが、TypeScript学習から Reactの学習に主眼が移る」と評価。ユーザーの判断で、Reactは本プロジェクトの延長ではなく別プロジェクトとして切り離す方針に決定。
  - 残り8項目(ゴーストピース/スマホ対応(ボタン)/ホールド機能/7-bagランダマイザ+ネクスト表示/効果音/スマホ対応(スワイプ・フリック)/ウォールキック/T-spin判定)を、実装の重さ(既存関数の再利用で済むか、新しいアルゴリズム的ロジックが要るか、他の項目が前提になるか)の観点で評価し、この順序で着手する方針をユーザーと合意。
  - `tetris-ts-learning-plan.md`に「11. 任意課題のロードマップ(第4マイルストーン完了後)」を新設し、上記8項目の重さ・備考を表形式で整理、Reactを別プロジェクト扱いとする理由を明記。
- 詰まった点(JS由来 / TS由来 / 環境由来): なし。
- 新しく理解した概念: (該当なし。計画相談のみ)
- 次回やること: ロードマップの1番目(ゴーストピース)から着手するかどうかユーザーと相談する。
